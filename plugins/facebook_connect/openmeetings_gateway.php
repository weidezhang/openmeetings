...
class openmeetings_gateway {
	...
	/**
	 * Login the user but set the landing zone to use the dashboard
	 */
	function openmeetings_setUserObjectMainLandingZone($username, $firstname, $lastname, 
			$profilePictureUrl, $email, $userId, $systemType) {
	    global $CFG;
	    
	    $restService = new openmeetings_rest_service();
	    
	    $url = $this->getUrl()."/services/UserService/setUserObjectMainLandingZone?" .
					"SID=".$this->session_id .
					"&username=".urlencode($username) .
					"&firstname=".urlencode($firstname) .
					"&lastname=".urlencode($lastname) .
